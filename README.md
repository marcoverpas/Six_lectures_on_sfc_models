...
We can calculate the (quasi) steady-state solution for national income `$Y$` by observing that in the steady state there must be no saving (`$C=YD$`) and household holdings of bills are stable (`$B_{h,−1}=B_h=B_h^{*}$`). Using these conditions and equations (2) and (3) in `$Y$`, we obtain:
