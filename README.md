...
We can calculate the (quasi) steady state solution for national income *Y* by observing that in the steady state there must be no saving and household holdings of bills are stable:  
...
We can check whether the model actually achieves the steady state as calculated above. The plot below shows that it does:
...
The baseline scenario is then compared with alternative scenarios, generated by shocks to exogenous variables and/or parameters. For instance, we may wonder what happens when the interest rate grows: 
